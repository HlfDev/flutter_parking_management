...
void main() {
  ...
  test(
    'when the call to [ParkingLotRepositoryImpl] method [saveParkingLotNewSpace] is successful, should return [Right] with a [null]',
    () async {
      //Arrange
      when(() => parkingLotRepositoryImpl.saveParkingLotNewSpace(
          parkingSpaceEntityMock)).thenAnswer((_) async => const Right(null));

      //Act
      final result =
          await saveParkingLotNewSpaceUseCase(parkingSpaceEntityMock);

      //Assert
      verify(() => parkingLotRepositoryImpl
          .saveParkingLotNewSpace(parkingSpaceEntityMock)).called(1);
      expect(result, const Right(null));
    },
  );

  test(
    'when the call to [ParkingLotRepositoryImpl] method [saveParkingLotNewSpace] is unsuccessful, should return [Left] with a [Failure]',
    () async {
      //Arrange
      when(() => parkingLotRepositoryImpl.saveParkingLotNewSpace(
          parkingSpaceEntityMock)).thenAnswer((_) async => Left(failureMock));
