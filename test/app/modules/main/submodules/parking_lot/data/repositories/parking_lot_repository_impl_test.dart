...
void main() {
  ...
  setUpAll(() {
    ...
    parkingLotRepositoryImpl =
        ParkingLotRepositoryImpl(parkingLotLocalDatasourceImpl);
  });
  ...
  group('[ParkingLotRepositoryImpl]', () {
    group(
      ...
      () {
        test(
          'and when the call is successful, should return [Right] with [List<ParkingSpaceEntity>]',
          () async {
            //Arrange
            ...
            //Act
            final result =
                await parkingLotRepositoryImpl.getListOfParkingLotSpace();

            //Assert
            verify(() =>
                    parkingLotLocalDatasourceImpl.getListOfParkingLotSpace())
                .called(1);
        ...
        test(
            'and when the call is unsuccessful, should return [Left] with a [Failure]',
            () async {
          //Arrange
          when(() => parkingLotLocalDatasourceImpl.getListOfParkingLotSpace())
          ...
          //Assert
          verify(() => parkingLotLocalDatasourceImpl.getListOfParkingLotSpace())
              .called(1);
    ...
    group('when the datasource method [saveParkingLotNewSpace] is called', () {
      test('and when the call is successful, should return [Right] with [null]',
          () async {
        //Arrange
        when(() => parkingLotLocalDatasourceImpl.saveParkingLotNewSpace(
            parkingSpaceEntityMock)).thenAnswer((_) async => returnsNormally);

        //Act
        final result = await parkingLotRepositoryImpl
            .saveParkingLotNewSpace(parkingSpaceEntityMock);

        //Assert
        verify(
          () => parkingLotLocalDatasourceImpl
              .saveParkingLotNewSpace(parkingSpaceEntityMock),
        ).called(1);
      ...
      test(
          'and when the call is unsuccessfull, should return [Left] with a [Failure]',
          () async {
        //Arrange
        when(() => parkingLotLocalDatasourceImpl.saveParkingLotNewSpace(
            parkingSpaceEntityMock)).thenThrow(exceptionMock);
    ...
    group('when the datasource method [removeParkingLotSpaceById] is called',
      ...
      test(
        ...
        //Arrange
        when(() => parkingLotLocalDatasourceImpl.removeParkingLotSpaceById(
            parkingSpaceEntityMock.id)).thenThrow(exceptionMock);
